...
### 4. The Setup: Vacuum, Slits, and Kinematics
Imagine a thermal chamber placed in a perfect vacuum (to prevent air resistance and Brownian scattering). The chamber features two tiny slits on opposite sides at height $H$. Once a particle exits either slit, thermodynamics ends and deterministic kinematics takes over. We calculate the time of flight using Newton's second equation of motion:
...
To construct a seamless continuous distribution, the central gas cannon is designed to have a thickness of exactly one histogram bin ($\delta$). Particles are emitted from the left and right slits located at the edges of this single bin. 

Why do we do this? If the container were arbitrary in size, there would be a massive, unnatural physical gap in the center of our statistical bell curve. By setting the container width to exactly one bin width, we guarantee that only the exact central bin ($x=0$) remains unoccupied. The particles naturally stack in the adjacent bins, allowing the rest of the Normal distribution to form symmetrically and seamlessly.

Furthermore, the vertical bars represent mathematical frequency tallies, not physical solid obstacles. Falling particles visually pass through these transparent statistical bars to reach the detector floor, and the bars are allowed to scale vertically past the height of the cannon without any physical interference.
